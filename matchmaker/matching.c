...
void matching(int* col_ptrs, int* col_ids, int* match, int* row_match, int n, int m, int matching_id, int cheap_id, double relabel_period) {
	int* row_ptrs;
	int* row_ids;
	int i, sp, ep, row;
	...
	for (i = 0; i < n; i++) 
		match[i] = -1;
	if(cheap_id == -1) /*an initial matching was given*/
	{
		for (i=0; i < m; i++)
			if(row_match[i] > -1)
				match[row_match[i]] = i;
	...
	if(matching_id >= do_hk || (cheap_id != do_old_cheap && cheap_id > 0)) {
